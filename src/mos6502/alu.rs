...
pub const fn add_with_carry(register: u8, value: u8, carry: bool) -> (u8, bool, bool) {
    ...
    let overflow: bool = (result ^ register) & (result ^ value) & 0b1000_0000 != 0;

    (result, carry, overflow)
}

pub const fn add_decimal_with_carry(register: u8, value: u8, carry: bool) -> (u8, bool, bool) {
    const fn lo(value: u8) -> u8 {
        (value & 0b0000_1111) >> 0
    }
...
fn adc_decimal() {
    assert_eq!(
        add_decimal_with_carry(0x09, 0x09, true),
        (0x19, false, false)
    );
    assert_eq!(
        add_decimal_with_carry(0x98, 0x01, true),
        (0x00, true, false)
    );
    assert_eq!(
        add_decimal_with_carry(0x79, 0x00, true),
        (0x80, false, true)
    );
    assert_eq!(
        add_decimal_with_carry(0x24, 0x56, false),
        (0x80, false, true)
    );
    assert_eq!(
        add_decimal_with_carry(0x82, 0x93, false),
        (0x75, true, true)
    );
    // out of bound / invalid input: C, but no V (both 80, f0 and d0 are negative)
    assert_eq!(
        add_decimal_with_carry(0x80, 0xf0, false),
        (0xd0, true, false)
    );
    // overflow first nibble carries 1, not 2, into high byte
    assert_eq!(
        add_decimal_with_carry(0x0f, 0x0f, true),
        (0x15, false, false)
    );
}
...
pub const fn sub_with_carry(register: u8, value: u8, carry: bool) -> (u8, bool, bool) {
...
pub const fn sub_decimal_with_carry(register: u8, value: u8, carry: bool) -> (u8, bool, bool) {
    const fn lo(value: u8) -> u8 {
        (value & 0b0000_1111) >> 0
    }
    const fn hi(value: u8) -> u8 {
    ...
    let overflow: bool = (binary_result ^ register) & (binary_result ^ value) & 0b1000_0000 != 0;
...
fn sbc_decimal() {
    ...
    assert_eq!(sub_decimal_with_carry(0x80, 0x80, true), (0x0, true, true)); // V=0 virtual6502
                                                                             ...
                                                                             // out of bound / invalid input: C, but no V (both 80, f0 and d0 are negative)
    assert_eq!(
        sub_decimal_with_carry(0x80, 0xf0, false),
        (0x29, false, false)
    );
    // overflow first nibble carries 1, not 2, into high byte
