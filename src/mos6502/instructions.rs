...
use crate::mos6502::{stack_pull, stack_push};
...
fn by_acc<AO: AccOp, AM: UseMode + UseValue>(
...
fn by_ref<RO: RefOp, AM: UseMode + UseReference + UseValue>(
    registers: &mut Registers,
    memory: &mut dyn MemoryBus,
) {
...
fn compare<const REGISTER: char, AM: UseMode + UseValue>(
    registers: &mut Registers,
    memory: &mut dyn MemoryBus,
) {
    ...
    let (result, carry, _overflow) = alu::sub_with_carry(lhs, rhs, CARRY);
    let negative = result & 0b0_1000_0000 != 0;
...
fn test_cpx() {
    ...
    compare::<'X', UseImmediate>(&mut regs, &mut mem);
    assert!(!regs.p.has::<'Z'>());
    assert!(regs.p.has::<'N'>());
...
fn branch<const FLAG: char, const SET: bool, AM: UseMode>(
    registers: &mut Registers,
    memory: &mut dyn MemoryBus,
) {
...
fn set_flag<const FLAG: char, const SET: bool, AM: UseMode>(
    registers: &mut Registers,
    _: &mut dyn MemoryBus,
) {
...
fn load<const REGISTER: char, AM: UseMode + UseValue>(
    registers: &mut Registers,
    memory: &mut dyn MemoryBus,
) {
    let value = AM::get_value(registers, memory);
...
fn transfer<const FROM: char, const TO: char, AM: UseMode>(
    registers: &mut Registers,
    _: &mut dyn MemoryBus,
) {
...
fn push_register<const REGISTER: char, AM: UseMode>(
    registers: &mut Registers,
    memory: &mut dyn MemoryBus,
) {
...
fn store<const REGISTER: char, AM: UseMode + UseAddress>(
    registers: &mut Registers,
    memory: &mut dyn MemoryBus,
) {
    const fn value<const REGISTER: char>(registers: &Registers) -> u8 {
...
pub fn handle_interrupt<const VECTOR: u16>(registers: &mut Registers, memory: &mut dyn MemoryBus) {
...
use Mnemonic::*;
const UND: Instruction = Instruction::new(
    Mnemonic::UND,
...
const INSTRUCTIONS: [Instruction; 256] = [
    ...
    Instruction::new(
        ...
        by_acc::<Ora, UseIndexedIndirectX>,
    ),
    UND,
    UND,
    ...
    Instruction::new(
        ...
        by_ref::<ShiftLeft<false>, UseZeroPage>,
    ),
    UND,
    Instruction::new(
        PHP,
        AddressingMode::Implied,
        push_register::<'p', UseImplied>,
    ), // 0x08
    Instruction::new(ORA, AddressingMode::Immediate, by_acc::<Ora, UseImmediate>),
    Instruction::new(
        ASL,
        AddressingMode::Accumulator,
        by_ref::<ShiftLeft<false>, UseAccumulator>,
    ),
    UND,
    ...
    Instruction::new(
        ...
        by_ref::<ShiftLeft<false>, UseAbsolute>,
    ),
    UND,
    Instruction::new(
        BPL,
        AddressingMode::Relative,
        branch::<'n', false, UseRelative>,
    ), // 0x10
    Instruction::new(
        ORA,
        AddressingMode::IndirectIndexedY,
        by_acc::<Ora, UseIndirectIndexedY>,
    ),
    UND,
    UND,
    ...
    Instruction::new(
        ...
        by_acc::<Ora, UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        ASL,
        AddressingMode::ZeroPageX,
        by_ref::<ShiftLeft<false>, UseZeroPageWith<'X'>>,
    ),
    UND,
    Instruction::new(
        CLC,
        AddressingMode::Implied,
        set_flag::<'C', false, UseImplied>,
    ), // 0x18
    Instruction::new(
        ORA,
        AddressingMode::AbsoluteY,
        by_acc::<Ora, UseAbsoluteWith<'Y'>>,
    ),
    ...
    Instruction::new(
        ...
        by_acc::<Ora, UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        ASL,
        AddressingMode::AbsoluteX,
        by_ref::<ShiftLeft<false>, UseAbsoluteWith<'X'>>,
    ),
    UND,
    ...
    Instruction::new(
        AND,
        AddressingMode::IndexedIndirectX,
        by_acc::<And, UseIndexedIndirectX>,
    ),
    UND,
    UND,
    ...
    Instruction::new(
        ROL,
        AddressingMode::ZeroPage,
        by_ref::<ShiftLeft<true>, UseZeroPage>,
    ),
    UND,
    ...
    Instruction::new(
        ROL,
        AddressingMode::Accumulator,
        by_ref::<ShiftLeft<true>, UseAccumulator>,
    ),
    UND,
    ...
    Instruction::new(
        ROL,
        AddressingMode::Absolute,
        by_ref::<ShiftLeft<true>, UseAbsolute>,
    ),
    UND,
    Instruction::new(
        BMI,
        AddressingMode::Relative,
        branch::<'N', true, UseRelative>,
    ), // 0x30
    Instruction::new(
        AND,
        AddressingMode::IndirectIndexedY,
        by_acc::<And, UseIndirectIndexedY>,
    ),
    UND,
    UND,
    UND,
    Instruction::new(
        AND,
        AddressingMode::ZeroPageX,
        by_acc::<And, UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        ROL,
        AddressingMode::ZeroPageX,
        by_ref::<ShiftLeft<true>, UseZeroPageWith<'X'>>,
    ),
    UND,
    Instruction::new(
        SEC,
        AddressingMode::Implied,
        set_flag::<'C', true, UseImplied>,
    ), // 0x38
    Instruction::new(
        AND,
        AddressingMode::AbsoluteY,
        by_acc::<And, UseAbsoluteWith<'Y'>>,
    ),
    ...
    Instruction::new(
        AND,
        AddressingMode::AbsoluteX,
        by_acc::<And, UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        ROL,
        AddressingMode::AbsoluteX,
        by_ref::<ShiftLeft<true>, UseAbsoluteWith<'X'>>,
    ), //0x3e
    UND,
    ...
    Instruction::new(
        EOR,
        AddressingMode::IndexedIndirectX,
        by_acc::<Eor, UseIndexedIndirectX>,
    ),
    UND, // 0x42
    UND, // 0x43
    UND, // 0x44
    ...
    Instruction::new(
        LSR,
        AddressingMode::ZeroPage,
        by_ref::<ShiftRight<false>, UseZeroPage>,
    ),
    UND, // 0x47
    Instruction::new(
        PHA,
        AddressingMode::Implied,
        push_register::<'A', UseImplied>,
    ),
    Instruction::new(EOR, AddressingMode::Immediate, by_acc::<Eor, UseImmediate>),
    Instruction::new(
        LSR,
        AddressingMode::Accumulator,
        by_ref::<ShiftRight<false>, UseAccumulator>,
    ),
    UND, // 0x4b
    ...
    Instruction::new(
        LSR,
        AddressingMode::Absolute,
        by_ref::<ShiftRight<false>, UseAbsolute>,
    ),
    UND, // 0x4f
    Instruction::new(
        BVC,
        AddressingMode::Relative,
        branch::<'V', false, UseRelative>,
    ),
    Instruction::new(
        EOR,
        AddressingMode::IndirectIndexedY,
        by_acc::<Eor, UseIndirectIndexedY>,
    ),
    UND, // 0x52
    UND, // 0x53
    UND, // 0x54
    Instruction::new(
        EOR,
        AddressingMode::ZeroPageX,
        by_acc::<Eor, UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        LSR,
        AddressingMode::ZeroPageX,
        by_ref::<ShiftRight<false>, UseZeroPageWith<'X'>>,
    ),
    UND, // 0x57
    Instruction::new(
        CLI,
        AddressingMode::Implied,
        set_flag::<'I', false, UseImplied>,
    ),
    Instruction::new(
        EOR,
        AddressingMode::AbsoluteY,
        by_acc::<Eor, UseAbsoluteWith<'Y'>>,
    ),
    UND, // 0x5a
    UND, // 0x5b
    UND, // 0x5c
    Instruction::new(
        EOR,
        AddressingMode::AbsoluteX,
        by_acc::<Eor, UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        LSR,
        AddressingMode::AbsoluteX,
        by_ref::<ShiftRight<false>, UseAbsoluteWith<'X'>>,
    ),
    UND, // 0x5f
    ...
    Instruction::new(
        ADC,
        AddressingMode::IndexedIndirectX,
        by_acc::<Adc, UseIndexedIndirectX>,
    ),
    UND, // 0x62
    UND, // 0x63
    ...
    Instruction::new(
        ROR,
        AddressingMode::ZeroPage,
        by_ref::<ShiftRight<true>, UseZeroPage>,
    ),
    UND, // 0x67
    ...
    Instruction::new(
        ROR,
        AddressingMode::Accumulator,
        by_ref::<ShiftRight<true>, UseAccumulator>,
    ),
    UND, // 0x6b
    ...
    Instruction::new(
        ROR,
        AddressingMode::Absolute,
        by_ref::<ShiftRight<true>, UseAbsolute>,
    ),
    UND, // 0x6f
    Instruction::new(
        BVS,
        AddressingMode::Relative,
        branch::<'V', true, UseRelative>,
    ),
    Instruction::new(
        ADC,
        AddressingMode::IndirectIndexedY,
        by_acc::<Adc, UseIndirectIndexedY>,
    ),
    UND, // 0x72
    UND, // 0x73
    ...
    Instruction::new(
        ADC,
        AddressingMode::ZeroPageX,
        by_acc::<Adc, UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        ROR,
        AddressingMode::ZeroPageX,
        by_ref::<ShiftRight<true>, UseZeroPageWith<'X'>>,
    ),
    UND, // 0x77
    Instruction::new(
        SEI,
        AddressingMode::Implied,
        set_flag::<'I', true, UseImplied>,
    ),
    Instruction::new(
        ADC,
        AddressingMode::AbsoluteY,
        by_acc::<Adc, UseAbsoluteWith<'Y'>>,
    ),
    ...
    Instruction::new(
        ADC,
        AddressingMode::AbsoluteX,
        by_acc::<Adc, UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        ROR,
        AddressingMode::AbsoluteX,
        by_ref::<ShiftRight<true>, UseAbsoluteWith<'X'>>,
    ),
    UND, // 0x7f
    UND, // 0x80
    Instruction::new(
    ...
    Instruction::new(
        DEY,
        AddressingMode::Implied,
        dec_register::<'Y', UseImplied>,
    ),
    UND, // 0x89
    Instruction::new(
        TXA,
        AddressingMode::Implied,
        transfer::<'X', 'A', UseImplied>,
    ),
    UND, // 0x8b
    ...
    Instruction::new(
        BCC,
        AddressingMode::Relative,
        branch::<'c', false, UseRelative>,
    ),
    Instruction::new(
        STA,
        AddressingMode::IndirectIndexedY,
        store::<'a', UseIndirectIndexedY>,
    ),
    UND, // 0x92
    UND, // 0x93
    Instruction::new(
        STY,
        AddressingMode::ZeroPageX,
        store::<'y', UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        STA,
        AddressingMode::ZeroPageX,
        store::<'a', UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        STX,
        AddressingMode::ZeroPageY,
        store::<'x', UseZeroPageWith<'Y'>>,
    ),
    UND, // 0x97
    Instruction::new(
        TYA,
        AddressingMode::Implied,
        transfer::<'Y', 'A', UseImplied>,
    ),
    Instruction::new(
        STA,
        AddressingMode::AbsoluteY,
        store::<'A', UseAbsoluteWith<'Y'>>,
    ),
    Instruction::new(
        TXS,
        AddressingMode::Implied,
        transfer::<'X', 'S', UseImplied>,
    ),
    UND, // 0x9b
    ...
    Instruction::new(
        BCS,
        AddressingMode::Relative,
        branch::<'c', true, UseRelative>,
    ),
    Instruction::new(
        LDA,
        AddressingMode::IndirectIndexedY,
        load::<'a', UseIndirectIndexedY>,
    ),
    UND, // 0xb2
    UND, // 0xb3
    Instruction::new(
        LDY,
        AddressingMode::ZeroPageX,
        load::<'Y', UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        LDA,
        AddressingMode::ZeroPageX,
        load::<'a', UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        LDX,
        AddressingMode::ZeroPageY,
        load::<'x', UseZeroPageWith<'Y'>>,
    ),
    UND, // 0xb7
    Instruction::new(
        CLV,
        AddressingMode::Implied,
        set_flag::<'V', false, UseImplied>,
    ),
    Instruction::new(
        LDA,
        AddressingMode::AbsoluteY,
        load::<'a', UseAbsoluteWith<'Y'>>,
    ),
    Instruction::new(
        TSX,
        AddressingMode::Implied,
        transfer::<'S', 'X', UseImplied>,
    ),
    UND, // 0xbb
    Instruction::new(
        LDY,
        AddressingMode::AbsoluteX,
        load::<'Y', UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        LDA,
        AddressingMode::AbsoluteX,
        load::<'a', UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        LDX,
        AddressingMode::AbsoluteY,
        load::<'X', UseAbsoluteWith<'Y'>>,
    ),
    UND, // 0xbf
    ...
    Instruction::new(
        DEC,
        AddressingMode::ZeroPage,
        by_ref::<Decrement, UseZeroPage>,
    ),
    UND, // 0xc7
    Instruction::new(
        INY,
        AddressingMode::Implied,
        inc_register::<'Y', UseImplied>,
    ),
    Instruction::new(CMP, AddressingMode::Immediate, compare::<'A', UseImmediate>),
    Instruction::new(
        DEX,
        AddressingMode::Implied,
        dec_register::<'X', UseImplied>,
    ),
    UND, // 0xcb
    ...
    Instruction::new(
        DEC,
        AddressingMode::Absolute,
        by_ref::<Decrement, UseAbsolute>,
    ),
    UND, // 0xcf
    Instruction::new(
        BNE,
        AddressingMode::Relative,
        branch::<'z', false, UseRelative>,
    ),
    Instruction::new(
        CMP,
        AddressingMode::IndirectIndexedY,
        compare::<'A', UseIndirectIndexedY>,
    ),
    UND, // 0xd2
    UND, // 0xd3
    UND, // 0xd4
    Instruction::new(
        CMP,
        AddressingMode::ZeroPageX,
        compare::<'A', UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        DEC,
        AddressingMode::ZeroPageX,
        by_ref::<Decrement, UseZeroPageWith<'X'>>,
    ),
    UND, // 0xd7
    Instruction::new(
        CLD,
        AddressingMode::Implied,
        set_flag::<'D', false, UseImplied>,
    ),
    Instruction::new(
        CMP,
        AddressingMode::AbsoluteY,
        compare::<'A', UseAbsoluteWith<'Y'>>,
    ),
    UND, // 0xda
    UND, // 0xdb
    UND, // 0xdc
    Instruction::new(
        CMP,
        AddressingMode::AbsoluteX,
        compare::<'A', UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        DEC,
        AddressingMode::AbsoluteX,
        by_ref::<Decrement, UseAbsoluteWith<'X'>>,
    ),
    UND, // 0xdf
    ...
    Instruction::new(
        SBC,
        AddressingMode::IndexedIndirectX,
        by_acc::<Sbc, UseIndexedIndirectX>,
    ),
    UND, // 0xe2
    UND, // 0xe3
    ...
    Instruction::new(
        INC,
        AddressingMode::ZeroPage,
        by_ref::<Increment, UseZeroPage>,
    ),
    UND, // 0xe7
    Instruction::new(
        INX,
        AddressingMode::Implied,
        inc_register::<'X', UseImplied>,
    ),
    ...
    Instruction::new(
        INC,
        AddressingMode::Absolute,
        by_ref::<Increment, UseAbsolute>,
    ),
    UND, // 0xef
    Instruction::new(
        BEQ,
        AddressingMode::Relative,
        branch::<'Z', true, UseRelative>,
    ),
    Instruction::new(
        SBC,
        AddressingMode::IndirectIndexedY,
        by_acc::<Sbc, UseIndirectIndexedY>,
    ),
    ...
    Instruction::new(
        SBC,
        AddressingMode::ZeroPageX,
        by_acc::<Sbc, UseZeroPageWith<'X'>>,
    ),
    Instruction::new(
        INC,
        AddressingMode::ZeroPageX,
        by_ref::<Increment, UseZeroPageWith<'X'>>,
    ),
    UND, // 0xf7
    Instruction::new(
        SED,
        AddressingMode::Implied,
        set_flag::<'D', true, UseImplied>,
    ),
    Instruction::new(
        SBC,
        AddressingMode::AbsoluteY,
        by_acc::<Sbc, UseAbsoluteWith<'Y'>>,
    ),
    ...
    Instruction::new(
        SBC,
        AddressingMode::AbsoluteX,
        by_acc::<Sbc, UseAbsoluteWith<'X'>>,
    ),
    Instruction::new(
        INC,
        AddressingMode::AbsoluteX,
        by_ref::<Increment, UseAbsoluteWith<'X'>>,
    ),
    UND, // 0xff
];
...
#[test]
fn step_by_step() {
    use crate::memory::{ram::RAM, Address, MemoryBus};
