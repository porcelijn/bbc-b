...
use bbc_b::memory::{ram::RAM, slice, Address, MemoryBus, PageDispatcher};
use bbc_b::mos6502::disassemble::disassemble_with_address;
use bbc_b::mos6502::{stop_at, stop_when, CPU};
...
fn interrogate_keyboard() {
    ...
    const MOS_CLIP: [u8; 17] = [
        ...
        0xa0,
        0x7f, // LDY #%01111111                     ; input on bit 7, output on bits 0 to 6
        ...
        0xae, 0x4f,
        0xfe, // LDX .systemVIARegisterANoHandshake ; read back ($80 + internal key number) if key pressed (or zero otherwise)
        0x60, // RTS
    ];
    ...
    let interrogate_keyboard = |cpu: &mut CPU, mem: &mut dyn MemoryBus| {
        ...
        while !stop(cpu, mem) {
            ...
            println!(
                "{dump:<30} | a:{} x:{} y:{} p:{:?}",
                cpu.registers.a, cpu.registers.x, cpu.registers.y, cpu.registers.p,
            );
        }
    };
...
fn loop_keyboard() {
    ...
    const MOS_CLIP: [u8; 68] = [
        ...
        0x8d, 0x43,
        0xfe, //     STA .systemVIADataDirectionRegisterA ; } set port A for input on bit 7 (all others are output bits)
        //                                          ; } i.e. we write to bits 0-6 of .systemVIARegisterANoHandshake, and get the result by reading bit 7
        0xa9, 0x03, //     LDA #3                               ; }
        0x8d, 0x40,
        0xfe, //     STA .systemVIARegisterB              ; } stop auto scanning of the keyboard
        //
        0xa9, 0x0f, //     LDA #15                              ; }
        0x8d, 0x4f,
        0xfe, //     STA .systemVIARegisterANoHandshake   ; } select a non-existent keyboard column 15 (0-9 only!)
        //
        0xa9, 0x01, //     LDA #1                               ; }
        0x8d, 0x4d,
        0xfe, //     STA .systemVIAInterruptFlagRegister  ; } cancel keyboard interrupts
        //
        0x8e, 0x4f,
        0xfe, //     STX .systemVIARegisterANoHandshake   ; select column X (9 max -> 0 min)
        0x2c, 0x4d,
        0xfe, //     BIT .systemVIAInterruptFlagRegister  ; check for any key in this column being pressed
        0xf0,
        0x21, //     BEQ .tryNextKeyboardColumn           ; if (bit 0 clear, i.e. there is no key pressed in this column) then branch
        0x8a, //     TXA                                  ; put column number (= first internal key number in column) in A
        //                                          ; inner loop: loop through the rows of the keyboard
        // .loopKeyboardRows
        0xd9, 0xdf,
        0x01, //     CMP .previousKeyPressedWhenReadingLastKey - .keyPressedInternalTable,Y  ; compare with first possible pressed key
        0x90,
        0x16, //     BCC .tryNextKeyboardRow              ; if (current key number is too low) then branch
        0x8d, 0x4f,
        0xfe, //     STA .systemVIARegisterANoHandshake   ; write which specific key to test
        0x2c, 0x4f,
        0xfe, //     BIT .systemVIARegisterANoHandshake   ; check for specific key being pressed
        0x10,
        0x0e, //     BPL .tryNextKeyboardRow              ; if (bit 7 is zero, i.e. key not pressed) then branch (go to next row)
        ...
        0xb0,
        0x13, //     BCS .finishKeyboardScanning          ; if (carry set, i.e. entry via OSBYTE 121/122) then branch (finished)
        0x48, //     PHA                                  ; Push A (key pressed)
        0x59, 0x00,
        0x00, //     EOR $0000,Y                          ; EOR with last key pressed (in $EC,$ED, or $EE depending on Y value)
        0x0a, //     ASL                                  ; shift left (to ignore the top bit)
        0xc9,
        0x01, //     CMP #1                               ; set carry if not the same as previous key pressed
        0x68, //     PLA                                  ; Pull A (key pressed)
        0xb0,
        0x09, //     BCS .finishKeyboardScanning          ; if (carry set, i.e. not the same as previous key pressed) then branch
        // .tryNextKeyboardRow
        0x18, //     CLC                                  ; }
        0x69,
        0x10, //     ADC #16                              ; } add 16 to check for key in next row
        0x10,
        0xe0, //     BPL .loopKeyboardRows                ; and do it again if 0 <= key < 128
        //     ; goto next column
